...
const portfolioData = {
  personal: {
    ...
    resumeLink: "/assets/pratul_deshpande_resume.pdf" 
  },
  education: [
    {
...
const GeminiChatModal = ({ isOpen, onClose }) => {
  ...
  const callGeminiApi = async (history) => {
    setIsLoading(true);
    // Standard and best practice way to access Vite environment variables.
  ...
  return (
    <div className="relative w-full min-h-screen bg-[#050505] text-white font-sans overflow-hidden flex flex-col items-center selection:bg-blue-500/30">
      ...
      <motion.div
        ...
        className="fixed bottom-24 right-8 z-50 group flex items-center gap-4"
      >
        {/* Persistent Text Box (Always Visible) */}
        <motion.div 
          initial={{ opacity: 0, x: 20 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ delay: 1.5 }}
          // Removed hidden md:block to allow visibility on mobile
          className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-xl px-4 py-2 text-sm text-white font-medium shadow-xl relative"
        >
          Ask me anything about Pratul!
          {/* Arrow pointing to button */}
          <div className="absolute top-1/2 -translate-y-1/2 -right-1.5 w-3 h-3 bg-white/10 border-t border-r border-white/20 transform rotate-45 backdrop-blur-xl"></div>
        </motion.div>
